...
class ShiftDetailsController extends GetxController {
  ...
  Future<void> fetchShifts() async {
    try {
      ...
      List<Shift> apiShifts = await ShiftDetails().GetAllShiftes(_authLogin!, result);

      shifts.value = apiShifts.map((s) => SiftDetailsModel(
        shiftID: s.ShiftID,
        shiftName: s.ShiftName,
        inTime: s.InTime,
        outTime: s.OutTime,
        // isShiftEndNextDay: s.shif,
        // isShiftStartsPreviousDay: s.IsShiftStartsPreviousDay,
        fullDayMinutes: s.FullDayMinutes,
        halfDayMinutes: s.HalfDayMinutes,
        isOTAllowed: s.IsOTAllowed,
        // oTStartMinutes: s.OutTime,
  ...
  Future<void> saveShift(SiftDetailsModel shift) async {
    try {
      if (_authLogin == null) {
        throw Exception('Authentication not initialized');
      }
      ...
      Shift apiShift = Shift()
        ...
        // ..IsShiftEndNextDay = shift.isShiftEndNextDay ?? false
        // ..IsShiftStartsPreviousDay = shift.isShiftStartsPreviousDay ?? false
        ..FullDayMinutes = shift.fullDayMinutes ?? 0
        ..HalfDayMinutes = shift.halfDayMinutes ?? 0
        ..IsOTAllowed = shift.isOTAllowed ?? false
        // ..OTStartMinutes = shift.oTStartMinutes ?? 0
  ...
  void updateSearchQuery(String query) {
    ...
    if (query.isEmpty) {
      ...
      filteredShifts.assignAll(
        shifts.where((shift) =>
            (shift.shiftName?.toLowerCase().contains(query.toLowerCase()) ?? false) ||
            (shift.inTime?.toLowerCase().contains(query.toLowerCase()) ?? false) ||
            (shift.outTime?.toLowerCase().contains(query.toLowerCase()) ?? false) ||
            (shift.lunchInTime?.toLowerCase().contains(query.toLowerCase()) ?? false) ||
            (shift.lunchOutTime?.toLowerCase().contains(query.toLowerCase()) ?? false)),
      );
    }
  }
