...
class _EmployeeSelectionDialogState extends State<EmployeeSelectionDialog> {
  final EmployeeSearchController controller = Get.put(EmployeeSearchController());
                ...
                return Column(
                  children: [
                    Expanded(
                      ...
                      child: ReusableTableAndCard(
                        data: List.generate(
                          ...
                          (index) => {
                            'Employee ID': controller.filteredEmployees[index].employeeID ?? '',
                            'Name': controller.filteredEmployees[index].employeeName ?? '',
                            'Enroll ID': controller.filteredEmployees[index].enrollID ?? '',
                            'Actions': controller.filteredEmployees[index].employeeID ?? '',
                          },
                        ),
                        headers: ['Employee ID', 'Name','Enroll ID','Actions'],
                        visibleColumns: ['Employee ID', 'Name', 'Enroll ID', 'Actions'],
                        onEdit: (row) {
                    ...
                    PaginationWidget(
                      ...
                      totalPages: (controller.totalRecords.value / controller.recordsPerPage.value).ceil(),
                      onFirstPage: () => controller.goToPage(0),
                      onPreviousPage: () => controller.previousPage(),
                      onNextPage: () => controller.nextPage(),
                      onLastPage: () => controller.goToPage(
                        (controller.totalRecords.value / controller.recordsPerPage.value).ceil() - 1,
                      ),
                      onItemsPerPageChange: (value) => controller.updateRecordsPerPage(value),
...
class _EmployeeFilterFormState extends State<EmployeeFilterForm> {
  ...
  void initState() {
    ...
    employeeIdController.text = widget.controller.searchEmployeeView.value.employeeID ?? '';
    enrollIdController.text = widget.controller.searchEmployeeView.value.enrollID ?? '';
    employeeNameController.text = widget.controller.searchEmployeeView.value.employeeName ?? '';
    selectedStatus = widget.controller.searchEmployeeView.value.employeeStatus ?? 1;
    ...
    try {
      ...
      dropdownItems = items.map((item) {
        ...
        if (item is DepartmentModel) {
          ...
          itemLabel = item.employeeTypeName;        } else if (item is String) {
          itemValue = item;
