...
class MainEmployeeTypeScreen extends StatelessWidget {
                  ...
                  Expanded(
                    child: ReusableTableAndCard(
                      data: List.generate(
                        ...
                        (index) => {
                          'Employee Type': controller.filteredEmployeeTypes[index].employeeTypeName,
                      ...
                      onEdit: (row) {
                        final employee = controller.filteredEmployeeTypes.firstWhere(
  ...
  void _showEmployeeTypeDialog(BuildContext context, [EmployeeTypeModel? employeeType]) {
    showCustomDialog(
      context: context,
      dialogContent: [
        EmployeeTypeDialog(
          controller: controller,
          employeeTypeModel: employeeType ?? EmployeeTypeModel(
            // employeeTypeId: 0,
  ...
  void _showDeleteConfirmationDialog(BuildContext context, EmployeeTypeModel employeeType) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text('Confirm Deletion'),
          content: const Text('Are you sure you want to delete this employee type?'),
          actions: <Widget>[
            TextButton(
              onPressed: () {
